...
export class Cursor {
  ...
  _fetch () {
    ...
    if (this.__result !== null) return this.__result
    ...
    // filter collection
    this.__result = new Lazy(this.__collection).filter(o => this.__query.test(o))
    let pipeline = []
   ...
   */
  next () {
    if (this.__stack === null) return undefined // done
   ...
   */
  hasNext () {
    if (this.__stack === null) return false // done
