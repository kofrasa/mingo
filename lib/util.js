...
export function each (obj, callback, ctx = null) {
  ...
  if (isArrayLike(obj)) {
    for (let i = 0, len = obj.length; i < len; i++) {
      if (callback.call(ctx, obj[i], i, obj) === false) break
    }
  } else {
    for (let k in obj) {
      if (has(obj, k)) {
        if (callback.call(ctx, obj[k], k, obj) === false) break
      }
    }
  }
}
...
/**
 * Reduce right any array-like object
 * @param obj
 * @param fn
 * @param accumulator
 * @returns {*}
 */
export function reduce (obj, fn, accumulator) {
  if (isArray(obj)) return obj.reduce(fn, accumulator)
  // array-like objects
  each(obj, (v, k) => accumulator = fn(accumulator, v, k))
  return accumulator
}
